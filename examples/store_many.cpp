...
bool do_operations(std::string hostname, uint16_t port,
  ...
  for (int i_request = 0; i_request < 20; ++i_request) {
    client.store("example_bucket", "example_key_" + std::to_string(i_request),
                 "hello", storers.wrap([&, i_request](std::error_code ec) {
                            if (ec) {
  ...
  storers.when_done([&] {
    ...
    auto removers = riak::make_completion_group([&] { client.managed_stop(); });
    for (int i_request = 0; i_request < 20; ++i_request) {
      client.remove("example_bucket",
...
int main(int argc, char* argv[]) {
  ...
  // Set the max number of connections to 20. This should have the effe
